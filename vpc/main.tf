...
#///////// Secret Rotation Lambda VPC Endpoint ////////

resource "aws_vpc_endpoint" "secret-rotation-vpc-endpoint" {
  vpc_id              = aws_vpc.vpc.id
  service_name        = "com.amazonaws.${local.region}.secretsmanager"
  vpc_endpoint_type   = "Interface"
  private_dns_enabled = true
  ...
  tags = merge(
    local.tags,
    {
      Name = "${var.arg_name}-secret-rotation-endpoint"
    }
  )
}

resource "aws_security_group" "secret-rotation-vpc-endpoint-sg" {
  name        = "${var.arg_name}-secret-rotation-endpoint-sg"
  description = "A security group for the secret rotation lambda to access ECS service"
  vpc_id      = aws_vpc.vpc.id
  tags = merge(
    local.tags,
    {
...
resource "aws_security_group_rule" "secret-rotation-ingress-fargate" {
  type                     = "ingress"
  description              = "Ingress from Fargate containers"
  from_port                = 443
  to_port                  = 443
  protocol                 = "tcp"
  source_security_group_id = aws_security_group.vpc-core-sg.id
  security_group_id        = aws_security_group.secret-rotation-vpc-endpoint-sg.id
}

resource "aws_security_group_rule" "secret-rotation-ingress-lambda-to-ecs-service" {
  type                     = "ingress"
  description              = "Ingress from Lambda Functions to Fargate containers"
  from_port                = 443
  to_port                  = 443
  protocol                 = "tcp"
  source_security_group_id = aws_security_group.secret-rotation-vpc-endpoint-sg.id
  security_group_id        = aws_security_group.secret-rotation-vpc-endpoint-sg.id
}

resource "aws_security_group_rule" "secret-rotation-egress-lambda-to-ecs-service" {
  type                     = "egress"
  description              = "Egress to Lambda Functions from Fargate containers"
  from_port                = 443
  to_port                  = 443
  protocol                 = "tcp"
  source_security_group_id = aws_security_group.secret-rotation-vpc-endpoint-sg.id
...
# If applied to the service then may be too permissive???
resource "aws_security_group_rule" "secret-rotation-egress-https" {
  type              = "egress"
  description       = "Egress for HTTPS"
  from_port         = 443
  to_port           = 443
