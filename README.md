...
If there are multiple web services on the application, you can add the additional domain to your certificate by adding the prefix name (eg. `internal.static`) to the variable `additional_address_list` see extensions.yml example below.  `Note: this is just the prefix, no need to add env.uktrade.digital`
