...
class TestWAFManagement:
    ...
    def test_waf_contains_both_secrets_during_rotation(self, rotator):
        ...
        with patch("boto3.client") as mock_boto_client:
            ...
            header_values = [
                s["ByteMatchStatement"]["SearchString"] for s in statements
            ]
...
class TestDistributionUpdates:
    ...
    def test_value_error_for_non_deployed_distribution(self, rotator):
        ...
        if not isinstance(excinfo.value, ValueError):
            pytest.fail(
                f"Expected ValueError, but got {type(excinfo.value).__name__} instead."
            )
...
class TestProcessCloudFrontDistributions:
    ...
    def test_all_distributions_already_have_header(self, rotator):
        ...
        actual_calls = (
            rotator.update_waf_acl.mock_calls + rotator.update_cf_distro.mock_calls
        )
    ...
    def test_some_distributions_missing_header(self, rotator):
        ...
        def mock_get_cf_distro_config(distro_id):
            if distro_id == "dist1":
                return {
                    "DistributionConfig": {
                        "Origins": {
                            "Items": [{"Id": "origin1", "CustomHeaders": {"Items": []}}]
                        }
                    }
                }
            return {
        ...
        for distro in matching_distributions:
            rotator.update_cf_distro.assert_any_call(
                distro["Id"], pending_secret["HEADERVALUE"]
            )
        ...
        actual_calls = (
            rotator.update_cf_distro.mock_calls + rotator.update_waf_acl.mock_calls
        )
        assert actual_calls == expected_calls
...
class TestSecretManagement:
    ...
    def test_new_secret_created_when_no_pending_exists(self, rotator):
        ...
        mock_service_client.get_random_password.return_value = {
            "RandomPassword": "new-secret"
        }
        ...
        mock_service_client.get_secret_value.assert_has_calls(
            [
                call(
                    SecretId="test-arn", VersionStage="AWSCURRENT"
                ),  # First call for AWSCURRENT
                call(
                    SecretId="test-arn",
    ...
    def test_awscurrent_not_found_logs_error(self, rotator):
        ...
        mock_logger.error.assert_called_with(
            "AWSCURRENT version does not exist for secret"
        )
...
class TestRotationProcess:
    ...
    def test_set_secret_updates_waf_first_when_all_distributions_have_header(
        self, rotator
    ):
        ...
        rotator.get_cf_distro_config = MagicMock(
            return_value=mock_get_distro_with_header
        )
        rotator.update_cf_distro = MagicMock()
        rotator.update_waf_acl = MagicMock()
    ...
    def test_secret_validates_all_origins_with_both_secrets(self, rotator):
        ...
        rotator.run_test_origin_access.assert_has_calls(
            expected_test_calls, any_order=True
        )
...
class TestErrorHandling:
    ...
    def test_handles_waf_update_failure_without_distribution_updates(self, rotator):
        ...
        mock_pending_secret = {
            "SecretString": json.dumps({"HEADERVALUE": "AWSPENDING"})
        }
        mock_current_secret = {
            "SecretString": json.dumps({"HEADERVALUE": "AWSCURRENT"})
        }
