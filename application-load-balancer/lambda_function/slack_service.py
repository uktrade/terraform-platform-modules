...
class SlackNotificationService:
    ...
    def send_test_failures(self, failures: List[Dict], environment: str, application: str, channel: str = None) -> None:
        ...
        try:
            ...
            response = self._send_message(channel or self.slack_channel, summary_text, message_blocks)
            ...
            if len(failure_text) > max_length:
                for i in range(0, len(failure_text), max_length):
                    ...
                    self._send_message(channel or self.slack_channel, thread_text, thread_ts=response['ts'])
                    logger.info("Additional failure details sent in thread")

        except Exception as e:
            logger.error(f"Failed to send Slack notification: {str(e)}")

    def _send_message(self, channel: str, text: str, blocks: List[Dict] = None, thread_ts: str = None) -> Dict:
        """
