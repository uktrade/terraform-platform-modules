...
class SecretRotator:
    def __init__(self, logger, **kwargs):
        ...
        waf_rule_priority = kwargs.get(
            "waf_rule_priority", os.environ.get("WAFRULEPRI")
        )
        try:
    ...
    def get_cloudfront_client(self) -> boto3.client:
        ...
        credentials = sts.assume_role(
            RoleArn=self.role_arn, RoleSessionName="rotation_session"
        )["Credentials"]
        return boto3.client(
            "cloudfront",
    ...
    def get_waf_acl(self) -> Dict[str, Any]:
        ...
        return client.get_web_acl(
            Name=self.waf_acl_name, Scope="REGIONAL", Id=self.waf_acl_id
        )
    ...
    def update_waf_acl(self, new_secret: str, prev_secret: str) -> None:
        ...
        new_rules = [rule] + [
            r
            for r in waf_acl["WebACL"]["Rules"]
            if r["Priority"] != self.waf_rule_priority
        ]
    ...
    def update_custom_headers(self, dist_config: Dict, header_value: str) -> bool:
        ...
        for origin in dist_config["DistributionConfig"]["Origins"]["Items"]:
            if (
                "CustomHeaders" not in origin
                or origin["CustomHeaders"]["Quantity"] == 0
            ):
                self.logger.info(
            ...
            for header in origin["CustomHeaders"]["Items"]:
                if header["HeaderName"] == self.header_name:
                    self.logger.info(
                        f"Updating existing custom header for origin: {origin['Id']}"
                    )
    ...
    def apply_distribution_update(
        self, client, distro_id: str, dist_config: Dict
    ) -> Dict:
        ...
        try:
            ...
            if status_code == 200:
                self.logger.info(
                    f"CloudFront distribution {distro_id} updated successfully"
                )
            else:
                self.logger.warning(
            ...
            self.logger.error(
                f"Error updating CloudFront distribution {distro_id}: {str(e)}"
            )
            raise
            ...
            for origin in dist_config["DistributionConfig"]["Origins"]["Items"]:
                ...
                if "Items" not in origin["CustomHeaders"]:
                    ...
                    if not header_found:
                        ...
                        all_have_header = (
                            False  # Mark this as needing update, since we added it
                        )
    ...
    def run_test_origin_access(self, url: str, secret: str) -> bool:
        try:
            ...
            self.logger.info(
                f"Testing URL, {url} - response code, {response.status_code}"
            )
            ...
            if response.status_code != 200:
                ...
                try:
                    ...
                    self.logger.error(
                        f"Could not log response content: {str(content_error)}"
                    )
            ...
            if hasattr(e, "response"):
                try:
                    self.logger.error(
                        f"Error Response Status Code: {e.response.status_code}"
                    )
                    ...
                    self.logger.error(
                        f"Could not log error response details: {str(log_err)}"
                    )
    ...
    def create_secret(self, service_client, arn, token):
        ...
        try:
            ...
            try:
                ...
                self.logger.error(
                    f"Failed to create AWSPENDING version for secret. Error: {e}"
                )
                raise
    ...
    def set_secret(self, service_client, arn, token):
        ...
        for distro in matching_distributions:
            ...
            if not self.is_distribution_deployed(distro["Id"]):
                self.logger.error(
                    f"Distribution Id, {distro['Id']} status is not Deployed."
                )
                raise ValueError(
                    f"Distribution Id, {distro['Id']} status is not Deployed."
                )
                ...
                try:
                    for s in secrets:
                        if self.run_test_origin_access("http://" + distro["Domain"], s):
                            ...
                            error_msg = (
                                f"Tests failed for URL, http://{distro['Domain']}"
                            )
                            self.logger.error(error_msg)
                            test_failures.append(
                                {
    ...
    def finish_secret(self, service_client, arn, token):
        ...
        self.logger.info(
            f"finishSecret: Successfully set AWSCURRENT stage to version {token}"
        )
