...
resource "aws_codebuild_project" "trigger_prod_copilot_codebuild" {
  for_each       = toset(local.triggers_another_pipeline ? [""] : [])
  name           = "${var.application}-${var.pipeline_name}-environment-prod-copilot"
  description    = "Triggers a CodeBuild job to run AWS Copilot environment commands"
  build_timeout  = 5
  service_role   = aws_iam_role.environment_pipeline_codebuild.arn
  ...
  environment {
    ...
    environment_variable {
      name  = "TRIGGERED_ACCOUNT_ROLE_ARN"
      value = local.triggered_pipeline_account_role
    }

    environment_variable {
      name  = "TRIGGERED_PIPELINE_AWS_PROFILE"
      value = local.triggered_pipeline_account_name
    }
  }
  ...
  source {
    type      = "NO_SOURCE"
    buildspec = file("${path.module}/buildspec-prod-copilot.yml")
  }
