...
class TestCreateDbUser:
    ...
    def test_new_user_can_connect_and_create_a_table(self):
        ...
        new_cursor.execute('CREATE TABLE new_user_table (id SERIAL PRIMARY KEY)')
        new_cursor.execute('INSERT INTO new_user_table VALUES (1)')
        
        new_cursor.execute('SELECT id FROM new_user_table')
    ...
    def test_that_executing_create_db_user_again_does_not_drop_new_user_tables(self, database_connection):
        ...
        cursor.execute('SELECT id FROM new_user_table')
