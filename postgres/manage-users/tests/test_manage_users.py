import pytest
import psycopg2
from manage_users.manage_users import create_or_update_user

class TestCreateOrUpdateUser:
    
    def test_create_or_update_user_creates_user(self, database_connection):
        ...
        create_or_update_user(database_connection, cursor, 'new_user', 'new_password', permissions)
    ...
    def test_executing_create_or_update_user_again_does_not_drop_new_user_tables(self, database_connection):
        cursor = database_connection.cursor()
        create_or_update_user(database_connection, cursor, 'new_user', 'new_password', ['SELECT'])
